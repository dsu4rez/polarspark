...
def test_equi_join_expression(spark):
    df1 = spark.createDataFrame([{"id1": 1, "name": "Alice"}, {"id1": 2, "name": "Bob"}])
...
def test_non_equi_join(spark):
    ...
    result = df1.join(df2, F.col("val1") > F.col("val2"), "inner").orderBy("val1", "val2").collect()
...
def test_complex_condition_join(spark):
    ...
    result = df1.join(df2, (F.col("id") == F.col("id")) & (F.col("a") == F.col("b"))).collect()
    ...
    result = df1.join(df2, (F.col("id1") == F.col("id2")) & (F.col("a") == F.col("b"))).collect()
