...
def test_row_number(spark):
    data = [{"grp": "A", "val": 10}, {"grp": "A", "val": 20}, {"grp": "B", "val": 5}]
    df = spark.createDataFrame(data)

    window = Window.partitionBy("grp").orderBy("val")
    result = (
        df.select(F.col("grp"), F.col("val"), F.row_number().over(window).alias("rn"))
        .orderBy("grp", "rn")
        .collect()
    )
...
def test_window_agg(spark):
    data = [{"grp": "A", "val": 10}, {"grp": "A", "val": 20}]
    df = spark.createDataFrame(data)

    window = Window.partitionBy("grp")
    result = df.select(F.sum("val").over(window).alias("sum_val")).collect()
