...
def test_ids(spark):
    ...
    df = spark.createDataFrame(data)
    result = df.select(
        F.monotonically_increasing_id().alias("id"), F.spark_partition_id().alias("pid")
    ).collect()
...
def test_binary_string(spark):
    ...
    # base64(col)
    result = (
        df.select(F.base64(F.col("text")).alias("b64"))
        .select(F.col("b64"), F.unbase64(F.col("b64")).alias("raw"))
        .collect()
    )
