...
def md5(col_name: Union[str, Column]) -> Column:
    ...
    return Column(expr.map_elements(lambda x: hashlib.md5(x.encode()).hexdigest() if x is not None else None, return_dtype=pl.Utf8))

def sha1(col_name: Union[str, Column]) -> Column:
    import hashlib
    expr = col_name._expr if isinstance(col_name, Column) else pl.col(col_name)
    return Column(expr.map_elements(lambda x: hashlib.sha1(x.encode()).hexdigest() if x is not None else None, return_dtype=pl.Utf8))
...
def sha2(col_name: Union[str, Column], num_bits: int) -> Column:
    ...
    def hash_fn(x):
        ...
        if num_bits == 256: return hashlib.sha256(x.encode()).hexdigest()
        if num_bits == 512: return hashlib.sha512(x.encode()).hexdigest()
...
def unix_timestamp(col_name: Optional[Union[str, Column]] = None, format: str = "yyyy-MM-dd HH:mm:ss") -> Column:
    if col_name is None:
        ...
        def transform_2_fn(arr):
            ...
            for i, v in enumerate(arr):
                ...
                if isinstance(out, Column):
                    ...
                    try: out = pl.select(out._expr).to_series()[0]
                    except: pass
                res.append(out)
            return res
...
def aggregate(col_name: Union[str, Column], initialValue: Any, merge: Any, finish: Any = None) -> Column:
    expr = col_name._expr if isinstance(col_name, Column) else pl.col(col_name)
    
    def get_val(v):
        if isinstance(v, Column):
            try: return pl.select(v._expr).to_series()[0]
            except: return v
        return v
    ...
    def agg_fn(arr):
        ...
        if isinstance(res, Column):
            try: res = pl.select(res._expr).to_series()[0]
            except: pass
        return res
...
def zip_with(left: Union[str, Column], right: Union[str, Column], f: Any) -> Column:
    ...
    def zip_fn(s):
        ...
        l_arr, r_arr = s["l"], s["r"]
        if l_arr is None or r_arr is None: return None
        res = []
    ...
    return Column(pl.struct([l_expr.alias("l"), r_expr.alias("r")]).map_elements(zip_fn, return_dtype=None))
    ...
    def extract(s):
        ...
        try:
            ...
            for part in clean_path.split("."):
                ...
                elif isinstance(d, list): d = d[int(part)]
                else: return None
...
def to_json(col_name: Union[str, Column]) -> Column:
    import json
    expr = col_name._expr if isinstance(col_name, Column) else pl.col(col_name)
    return Column(expr.map_elements(lambda x: json.dumps(x) if x is not None else None, return_dtype=pl.Utf8))

# Bitwise

def shiftLeft(col_name: Union[str, Column], n: int) -> Column:
    expr = col_name._expr if isinstance(col_name, Column) else pl.col(col_name)
    return Column(expr.map_elements(lambda x: x << n if x is not None else None, return_dtype=pl.Int64))
...
def hex(col_name: Union[str, Column]) -> Column:
    ...
    return Column(expr.map_elements(lambda x: format(x, 'X') if x is not None else None, return_dtype=pl.Utf8))
...
def regexp_extract(col_name: Union[str, Column], pattern: str, group: int = 1) -> Column:
    expr = col_name._expr if isinstance(col_name, Column) else pl.col(col_name)
...
def approx_count_distinct(col_name: Union[str, Column], rsd: float = 0.05) -> Column:
    expr = col_name._expr if isinstance(col_name, Column) else pl.col(col_name)
    return Column(expr.n_unique()) # Polars is already very fast, n_unique is usually fine
...
def map_from_arrays(keys: Union[str, Column], values: Union[str, Column]) -> Column:
    ...
    return Column(pl.struct([k_expr.alias("k"), v_expr.alias("v")]).map_elements(zip_map))
...
def months_between(date1: Union[str, Column], date2: Union[str, Column]) -> Column:
    ...
    diff = (expr1.dt.year() - expr2.dt.year()) * 12 + (expr1.dt.month() - expr2.dt.month())
...
def instr(col_name: Union[str, Column], substring: str) -> Column:
    ...
    return Column(pl.when(pos.is_null() | (pos == -1)).then(pl.lit(0)).otherwise(pos + 1))
...
def translate(col_name: Union[str, Column], matching: str, replacement: str) -> Column:
    ...
    return Column(expr.map_elements(lambda x: x.translate(trans_table) if x is not None else None, return_dtype=pl.Utf8))
