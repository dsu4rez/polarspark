...
class Column:
    ...
    def _resolve(
        ...
        if self._is_op:
            if self._op == "alias":
                return self._left._resolve(
                    left_ref, right_ref, left_alias, right_alias, left_cols, right_cols
                ).alias(self._right)
            if self._op == "getField":
                return self._left._resolve(
                    left_ref, right_ref, left_alias, right_alias, left_cols, right_cols
                ).struct.field(self._right)
            if self._op == "cast":
                return self._left._resolve(
                    left_ref, right_ref, left_alias, right_alias, left_cols, right_cols
                ).cast(self._right)
            ...
            l = (
                ...
                if isinstance(self._left, Column)
                else pl.lit(self._left)
            )
            r = (
                self._right._resolve(
                    left_ref, right_ref, left_alias, right_alias, left_cols, right_cols
                )
                if isinstance(self._right, Column)
                else pl.lit(self._right)
            )
            # ... ops ...
            ops = {
                "+": lambda a, b: a + b,
    ...
    def over(self, window: "Window") -> "Column":
        ...
        if hasattr(self, "_is_rank"):
            ...
            if window._order_by:
                ...
                rank_col = (
                    pl.col(first_col) if isinstance(first_col, str) else first_col
                )
    ...
    def getField(self, name: str) -> "Column":
        return self._init_op(
            Column(self._expr.struct.field(name)), "getField", self, name
        )
    ...
    def __add__(self, other: Any) -> "Column":
        return self._init_op(
            Column(
                self._expr
                + (other._expr if isinstance(other, Column) else pl.lit(other))
            ),
            "+",
            self,
            other,
    ...
    def desc(self) -> "Column":
        return Column(self._expr)  # Polars handle sort order in sort()
    ...
    def otherwise(self, value: Any) -> "Column":
        ...
        return Column(
            self._expr.otherwise(
                value._expr if isinstance(value, Column) else pl.lit(value)
            )
        )
