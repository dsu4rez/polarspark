...
class DataFrame:
    ...
    def filter(self, condition: Union[str, Column]) -> "DataFrame":
        if isinstance(condition, str):
            ...
            raise NotImplementedError(
                "String conditions in filter are not yet supported. Use Column expressions."
            )
    ...
    def orderBy(
        self, *cols: Union[str, Column], ascending: Union[bool, List[bool]] = True
    ) -> "DataFrame":
        exprs = []
        # Polars handles lists for descending
        is_descending = (
            not ascending if isinstance(ascending, bool) else [not a for a in ascending]
        )
    ...
    def join(
        self, other: "DataFrame", on: Union[str, List[str], Column], how: str = "inner"
    ) -> "DataFrame":
        ...
        if isinstance(on, Column):
            ...
            if getattr(on, "_is_equi_join_cond", False):
                l_match = on._l_ref == id(self) or (
                    self._alias and on._l_table == self._alias
                )
                r_match = on._r_ref == id(other) or (
                    other._alias and on._r_table == other._alias
                )

                if l_match and r_match:
                    if on._l_col == on._r_col:
                        res_df = self._df.join(
                            other._df, left_on=on._l_col, right_on=on._r_col, how=pl_how
                        )
                    else:
                ...
                # Check reversed
                l_match_rev = on._l_ref == id(other) or (
                    other._alias and on._l_table == other._alias
                )
                r_match_rev = on._r_ref == id(self) or (
                    self._alias and on._r_table == self._alias
                )
                if l_match_rev and r_match_rev:
                    if on._l_col == on._r_col:
                        res_df = self._df.join(
                            other._df, left_on=on._r_col, right_on=on._l_col, how=pl_how
                        )
                    else:
            ...
            if pl_how == "semi":
                ...
                res_df = pl.concat([matches, unmatched], how="diagonal").drop(
                    [left_id, right_id]
                )
            elif pl_how == "right":
                ...
                res_df = pl.concat(
                    [matches, unmatched_l, unmatched_r], how="diagonal"
            ...
            raise NotImplementedError(
                f"Join on Column expressions with '{how}' join is not yet supported."
            )
        ...
        if isinstance(on, list):
            ...
            for c in on:
                if isinstance(c, Column):
                    if c._is_simple_col and (
                        c._df_ref == id(other)
                        or (other._alias and c._table_name == other._alias)
                    ):
